...
import com.vertica.spark.util.query.{ColumnInfo, ColumnsTable, ComplexTypeInfo, ComplexTypesTable, TypesTable}
          ...
          val colDefsOrErrors: List[ConnectorResult[ColumnDef]] =
            (1 to rsmd.getColumnCount)
              .map(idx => {
                ...
                colDef.jdbcType match {
                  case java.sql.Types.ARRAY |
                       java.sql.Types.STRUCT =>
                    startQueryingVerticaComplexTypes(colDef, tableInfo.tableName, tableInfo.dbSchema, jdbcLayer)
                  case _ => Right(colDef)
                }
              }).toList
          colDefsOrErrors
