...
class StringParsingUtilsTest extends AnyFlatSpec {

  behavior of "VerticaSQLUtilsTest"

  it should "split a comma separated list" in {
    val result = StringParsingUtils.splitByComma("cat, cat dog, shark,")
  ...
  it should "split a comma separated list with parentheses" in {
    val result = StringParsingUtils.splitByComma("col1 (int, col2) (cat, ((dog))), shark")
    assert(result.length == 2)
    assert(result.head == "col1 (int, col2) (cat, ((dog)))")
    assert(result(1) == "shark")

    val result2 = StringParsingUtils.splitByComma("(col1 int, col2 cat dog,)")
    assert(result2.length == 1)
    assert(result2.head == "(col1 int, col2 cat dog,)")

    val result3 = StringParsingUtils.splitByComma(")(col1 (int, ()col2, shark)), cat, dog")
  ...
  it should "split a comma separated list with non matching parentheses" in {
    val result = StringParsingUtils.splitByComma("(col1 int, col2 cat dog,")
    assert(result.length == 1)
    assert(result.head == "(col1 int, col2 cat dog,")

    val result2 = StringParsingUtils.splitByComma(")col1 (int, (col2, shark)), cat, dog")
  ...
  it should "find the indices of the first matching parentheses" in {
    ...
    val (openParen, closeParen) = StringParsingUtils.findFirstParenGroupIndices(str)
    assert(openParen == 4)
