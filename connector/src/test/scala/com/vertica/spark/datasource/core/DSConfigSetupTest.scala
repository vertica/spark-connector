...
class DSConfigSetupTest extends AnyFlatSpec with BeforeAndAfterAll with MockFactory {
  ...
  it should "return None for AWS credentials provider when no Spark session is active" in {
    SparkSession.getActiveSession.foreach(_.close())

    DSConfigSetupUtils.getAWSCredentialsProvider(Map.empty) match {
      case Valid(None) => succeed
      case Valid(Some(_)) => fail("Expected no AWS credentials provider when no Spark session is active")
      case Invalid(errs) => fail("Expected valid None, got errors: " + errs.toString)
    }
  }
}
