...
class EndToEndTests(readOpts: Map[String, String]) extends AnyFlatSpec with BeforeAndAfterAll {
  ...
  it should "be able to handle the geometry type" in {
    val tableName1 = "dftest"
    val stmt = conn.createStatement()
    val n = 1
  ...
  it should "be able to handle the geography type" in {
    val tableName1 = "dftest"
    val stmt = conn.createStatement()
    val n = 1

    TestUtils.createTableBySQL(conn, tableName1, "create table " + tableName1 + " (f geography)")

    val insert = "insert into "+ tableName1 + " values(ST_GeographyFromText('POLYGON((1 2,3 4,2 3,1 2))'))"
    TestUtils.populateTableBySQL(stmt, insert, n)
