...
| merge_key                                      | String                                              | In order to execute a merge statement in our Spark Connector, the user needs to pass in a mergeKey, which will need to be a list of comma-separated column attributes to join the existing table and temporary table on. If this option is not populated, a write will be performed as usual without the use of a temporary table.                                                                                                                                                                                                                                                                           | No                                             |
...
| aws_enable_ssl                                 | "true" / "false"                                              | Whether SSL should be used. Defaults to always using SSL, only change this if using an endpoint that does not use https. This value cannot be set on the session level in Vertica, so must be set seperately. Use ALTER DATABASE <dbname> SET AWSEnableHttps=0 for this.                                                                                                                                                                                                                                                                                                                                                                        | No                                             |                        true |
