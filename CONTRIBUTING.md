...
Running this will run all unit tests and build the jar to target/[SCALA_VERSION]/spark-vertica-connector-assembly-3.0.4.jar
...
mkdir lib
cp ../../connector/target/scala-2.12/spark-vertica-connector-assembly-3.0.4.jar lib
sbt run
```
...
If you set the sparkVersion in build.sbt to 3.0.4, you will also need to use hadoop-hdfs version 2.7.0 when running `sbt run` to run the integration tests.
